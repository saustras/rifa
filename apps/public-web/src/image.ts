...
// If the browser cannot decode the image (rare HEIC on some Android browsers),
// we fall back to sending the original file untouched.
...
export const prepareImageForUpload = async (file: File): Promise<PreparedImage> => {
  ...
  try {
    ...
    // Fallback: send the original file as-is (server accepts heic/heif too).
    return {
      fileName: file.name,
